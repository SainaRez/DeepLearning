...
class Data:
    ...
    class LoadData:
        ...
        def get_path_download(input_path):
        ...
        def get_classes():
            ...
            self.files = tf.io.gfile.glob(str(self.data_dir) + '/*/*')
            ...
            print('Example file tensor:', self.files[0])

        # Dividing the data into train, eval and test
        def divide_data():
            
            train_files = self.files[:6400]
            val_files = self.files[6400: 6400 + 800]
            test_files = self.files[-800:]
